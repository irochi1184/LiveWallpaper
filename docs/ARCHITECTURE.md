...
WorkerWWallpaperHostはWS_POPUPを外してWS_CHILDを設定し、SetParentの戻り値と最終エラーを確認する。モニターのスクリーン座標をMapWindowPointsで親クライアント座標へ変換し、SetWindowPosで非アクティブ配置する。従来の構造ではトップレベルWorkerW内へ配置する。新しいWindows 11のraised desktopでは、ProgmanのWS_EX_NOREDIRECTIONBITMAPを検出し、Progmanの直接の子としてSHELLDLL_DefViewの直後へ配置する。これによりアイコンより下、背景WorkerWより上になる。背景WorkerWの子に入れると配置APIが成功しても時計が隠れるため、この構造では使わない。Explorerのウィンドウ自体を隠したり破棄したりしない。
